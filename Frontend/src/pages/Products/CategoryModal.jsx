...
const [rowsPerPage, setRowsPerPage] = useState(5);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        await axios.put(`${API_BASE_URL}/api/categories/edit/${editingId}`, data);
  ...
  open={open}
  onClose={onClose}
  fullWidth={false}       // Disables default full width behavior
  maxWidth={false}        // Disables default maxWidth constraint
  PaperProps={{
    sx: {
      width: { xs: "95vw", sm: "800px", md: "900px" }, // Responsive
      maxWidth: "95vw",       // Prevents overflow
      m: { xs: 1, sm: 2 },     // Margin from screen edge
      borderRadius: 2,
                ...
                filteredCategories
                  ...
                  .map((cat, index) => (
                    <TableRow key={cat.category_id} hover>
                      <TableCell sx={{fontWeight: 'bold', width: '25px', textAlign: 'center'}}>{page * rowsPerPage + index + 1}</TableCell>
                      ...
                      <TableCell>
                        <IconButton onClick={() => handleEdit(cat)}
                            sx={{color: primaryColor, border: `1px solid ${primaryColor}`, 
                            "&:hover": {
                            boxShadow: `0 0 8px ${primaryColor}`,
                          },}}
                        >
